...
@LastEditTime: 2023-12-21 16:50:40
@FilePath: /oh_my_fiftyone/core/importer/sgccgame_dataset_importer.py
@Description:
"""
...
class SGCCGameDatasetImporter(foud.LabeledImageDatasetImporter):
    ...
    def setup(self):
        self._imgs_path = get_all_file_path(
            self._dataset_dir,
...
def objs_infomap2foDetections(
    objs_info: dict, img_metadata: fom.ImageMetadata
) -> fol.Detections:
    result = []
    for k, v in objs_info.items():
        for obj in v:
            bbox, flag = normalization_xyxy(
                obj, img_metadata.width, img_metadata.height
            )
            if not flag:
                result.append(
                    fol.Detection(label=k, bounding_box=bbox, ori_data_prob=True)
                )
            else:
                result.append(
                    fol.Detection(label=k, bounding_box=bbox, ori_data_prob=False)
                )
...
def parse_sample_info(
    img_path,
) -> Tuple[Optional[fo.ImageMetadata], fol.Detections, dict]:
    anno_dict = {}
    ...
    if not os.path.exists(anno_path):
        ...
        anno_dict["chiebot_ID"] = (
            "game_" + md5sum(img_path)
            if not os.path.basename(img_path).startswith("game_")
            else os.path.splitext(os.path.basename(img_path))[0]
        )
    ...
    if anno.get("embedding", None) is not None:
        anno_dict["embedding"] = np.frombuffer(
            base64.b64decode(anno["embedding"].encode("utf-8"))
        )

    return img_meta, label_info, anno_dict


def generate_sgcc_sample(
    img_path, extra_attr: Optional[dict] = None
) -> Optional[fo.Sample]:
