...
@LastEditTime: 2023-12-08 17:16:45
    ...
    if xmls or txts:
        if len(xmls) > len(txts):
            ...
            if os.path.exists(os.path.join(label_dir, "obj.names")):
                ...
                logging.error(
                    "错误:猜不到{}是什么标签,看起来像yolo,但是不规范".format(label_dir)
                )
                print(
                    "错误:猜不到{}是什么标签,看起来像yolo,但是不规范".format(label_dir)
                )
...
def _parse_voc_deal_one(xml, ex_cls) -> Tuple[str, list]:
    return (
        os.path.basename(xml).split(".")[0],
        [
            x
            for x in voc.load_voc_detection_annotations(xml).to_detections().detections
            if x.label not in ex_cls
        ],
    )
...
def _parse_voc(xml_list, exclude_classes) -> Dict[str, list]:
    ...
    with fo.ProgressBar(
        ...
        with futures.ThreadPoolExecutor(48) as exec:
            tasks = (
                exec.submit(_parse_voc_deal_one, xml, exclude_classes)
                for xml in xml_list
            )
...
def _parse_yolo_deal_one(txt, classes, ex_cls) -> Tuple[str, list]:
    return (
        os.path.basename(txt).split(".")[0],
        [
            x
            for x in yolo.load_yolo_annotations(txt, classes).detections
            if x.label not in ex_cls
        ],
    )
...
def _parse_yolo(txt_list, classes, exclude_classes) -> Dict[str, list]:
    ...
    with fo.ProgressBar(
        ...
        with futures.ThreadPoolExecutor(48) as exec:
            tasks = (
                exec.submit(_parse_yolo_deal_one, txt, classes, exclude_classes)
                for txt in txt_list
            )
...
def _parse_coco(anno_json, exclude_classes) -> Dict[str, list]:
    (
        ...
        img_ids_map,
        anno,
    ) = coco.load_coco_detection_annotations(anno_json)
...
def parser_labels(
    label_dir, exclude_class=None, label_type=None
) -> Dict[str, List[fol.Detection]]:
