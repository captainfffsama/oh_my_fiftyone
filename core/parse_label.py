...
def _parse_voc(xml_list) -> Dict[str,list]:
    deal_one=lambda xml:(os.path.basename(xml).split(".")[0],voc.load_voc_detection_annotations(xml).to_detections().detections)
    result={}
    with fo.ProgressBar(
            total=len(xml_list), start_msg="标签文件解析进度:", complete_msg="解析完毕"
        ) as pb:
        with futures.ThreadPoolExecutor(48) as exec:
            tasks=[exec.submit(deal_one, xml) for xml in xml_list]
...
def _parse_yolo(txt_list,classes) -> Dict[str,list]:
    deal_one=lambda txt,classes:(os.path.basename(txt).split(".")[0],yolo.load_yolo_annotations(txt,classes).detections)
    result={}
    with fo.ProgressBar(
            total=len(txt_list), start_msg="标签文件解析进度:", complete_msg="解析完毕"
        ) as pb:
        with futures.ThreadPoolExecutor(48) as exec:
            tasks=[exec.submit(deal_one,txt,classes) for txt in txt_list]
...
def _parse_coco(anno_json) -> Dict[str,list]:
...
def parser_labels(label_dir,label_type=None) -> Dict[str,List[fol.Detection]]:
