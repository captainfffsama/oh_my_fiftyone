...
def print_time_deco(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            print(
                "\033[1;34m操作完成时间: {}, 操作耗时: {} 秒\033[0m".format(
...
@print_time_deco
def get_select_dv(
    txt_path: str = None, ordered: bool = False
) -> Optional[fo.DatasetView]:
    ...
    if dataset and session:
        if txt_path is not None:
            if os.path.exists(txt_path):
                ...
                return dataset.select_by("filepath", imgs_path, ordered=ordered)
        else:
...
def find_similar_img(
    ...
    dataset_name = (
        dataset.name if isinstance(dataset, focd.Dataset) else dataset.dataset_name
    )
    ...
    tmp = [
        (qdrant_point.payload["sample_id"], qdrant_point.score)
...
def split_dataset(
    ...
    assert (
        round(sum(split_ratio), 3) == 1
    ), "the sum of split_ratio must be 1, 别把数据搞丢了"  # 0.7 + 0.1 = 0.899999
    ...
    if class_list:
        ...
        while cls_info_temp:
            ...
            cls_data = dataset_temp.match(
                F("ground_truth.detections.label").contains([cls, ""])
            )
            ...
            temp_train, temp_val, temp_test = split_data_force(
                cls_path, split_ratio
            )  # 单纯按类别划分，不考虑跨类别均衡情况下，运行时间可以接受
            total_split = len(temp_train) + len(temp_val) + len(temp_test)
            assert (
                len(cls_path) == total_split
            ), "!!!!! {}  !!!!!当前类别:{}，划分前：{}，划分后：{}".format(
                len(cls_path), cls, len(cls_path), total_split
            )
            ...
            cls_info_temp = dict(
                sorted(cls_info_temp.items(), key=lambda x: x[1])
            )  # 重新排序
            visited_classes.add(cls)
            del cls_info_temp[cls]
    ...
    print(
        "划分之后的训练集:{}、验证集:{}、测试集:{}".format(
            len(split_train), len(split_val), len(split_test)
        )
    )
    if (
        sample_tag + "_train" in exists_tags
        or sample_tag + "_test" in exists_tags
        or sample_tag + "_val" in exists_tags
    ):
        print("---------------------------------------------", force_overwrite)
        if force_overwrite:
            print(
                "给你一次反悔的机会，是否要强制删除并覆盖{}、{}、{}字段，是：y/Y, 否：输入其他任意字符".format(
            ...
            logging.warning(
                "The target field already exists, please force_overwrite set to True"
            )
            return
    ...
    print(
        "\033[1;34m数据划分成功，相应字段为{}、{}、{}\033[0m".format(
            sample_tag + "_train", sample_tag + "_val", sample_tag + "_test"
        )
    )
