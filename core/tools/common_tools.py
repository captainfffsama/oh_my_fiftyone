...
def split_dataset(
    ...
    if class_list:
        ...
        visited_classes = set()
        with tqdm(total=len(cls_info)) as pbar:
            pbar.set_description("数据集划分进度")
            while cls_info:
                cls = list(cls_info.items())[0][0]  # 类别数量最少的cls
                num = list(cls_info.items())[0][1]
                ...
                cls_info = dict(sorted(cls_info.items(), key=lambda x: x[1]))  # 重新排序
                visited_classes.add(cls)
                del cls_info[cls]
                pbar.update(1)
